...
def insert_coin(coin):
  ...
  record = [datetime.now().strftime("%d.%m.%Y"), datetime.now().strftime("%H:%M:%S"), coin, get_total() + coin]
...
def insert_coin_test(coin, date):
  record = [date, datetime.now().strftime("%H:%M:%S"), coin, round(get_total() + coin, 2)]
